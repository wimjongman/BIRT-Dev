// Compex Systemhaus GmbH, Heidelberg, Germany
// Loetz GmbH & Co KG

module.exports = {
  someSidebar: {
    About: ['what-is-birt','overview-design','overview-architecture','overview-designer','overview-customization','overview-extensibility'],
    "Getting Started": ['getting-started','overview', 'installation','create-a-report', 'community'],
    Demos: [],
    Documentation: ['t_brief-editor-tour','t_grids','t_data-sources','t_data-sets','t_data-properties','t_cascading-styles','t_create-a-report','t_styles','t_tables','t_testing-reports','t_text-item','t_tools','t_visual-properties'],
    "Working Group": [],
    "Templates&Examples": ['template-introduction','template-sample-database'],
    ///Docusaurus: ['doc1', 'doc2', 'doc3', 'myTest'],
  },
};
